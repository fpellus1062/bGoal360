    ...
    $.ajax({ 
      ...
      success: function(lays) {
        ...
        for (var i=0; i < lays.length; i++) {
          elemento.label = lays[i].codigo+"- "+lays[i].version+'-'+lays[i].nombre+'-'+lays[i].descripcion+'-'+lays[i].observaciones+'-'+lays[i].campo_resumen;
...
window.Smart('#resumenlayout', class {
 ...
 function actualizar(nivel,ultimonivel) {
      ...
      $.ajax({ 
        ...
        dataType: 'json',
        cache: false, 
        data: { layout_id: idlayout.value[0].value,nivel_id: ultimonivel,version: listalay[indices[0]].version},  
        success: function(datosdist) {
          ...
          $("#progressbar").css('width', '100' + "%");distable.refresh();
      ...
      $.ajax({ 
        ...
        dataType: 'json',
        cache: false, 
        data: { empresa_id: '2', layout_id: idlayout.value[0].value,nivel_id: ultimonivel,version: listalay[indices[0]].version},  
        success: function(datospareto) {
          ...
          { label:columnaResumen[idlayout.value[0].value], dataField: 'total', dataType: 'string',align: 'right',formatFunction(settings) {
            if (settings.value == 0 ) {
              settings.template = `<b><span class="red">  ${columnaResumen[idlayout.value[0].value]}</span><b>`;
            } 
           
        }},
          { label:'#', dataField: 'posicion', dataType: 'string',align: 'right',formatFunction(settings) {
            if (settings.value == 0 ) {
          ...
          { label: 'Peso', dataField: 'percent', dataType: 'number',formatFunction(settings) {
            ...
            settings.template = `<span class="${className}">${value.toFixed(2)}%</span>`;
            if (value == 0 ) {
