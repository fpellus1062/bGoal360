...
exportBtn.addEventListener("doubleClick", function (event) {
...
var calculapesos = function (fila, modo) {
	...
	//Ggrid.setRowStyle(4,{fontFamily: "Poppins","background":COLORES_FONDO[fila.level]});
...
var operarhijos = function (fila, columna, ltotalanterior, ltotalnuevo) {
	...
	fila.children.forEach(function (hijos) {
		...
		grid.setCellValue(hijos.id, columna, redondea(lvalor));
...
var operarhijostotal = function (fila, ltotalanterior, ltotalnuevo) {
	...
	//grid.setCellValue(fila.id, COLUMNAS[0],redondea(ltotalnuevo));
	const unitario = redondea(
		parseFloat(ltotalnuevo) / parseFloat(gcuentaniveles[MAXIMONIVEL])
	);
	...
	fila.children.forEach(function (hijos) {
		...
		for (var i = 1; i <= 12; i++) {
			...
			grid.setCellValue(hijos.id, COLUMNAS[i], redondea(lvalor));
...
var operar = function (ltotalanterior, ltotalnuevo, fila, columna, modo) {
	...
	if (columna == "Tot") {
		...
		for (var i = 1; i <= 12; i++) {
			...
			valoresmaster.push(grid.rows[fila.id].data[COLUMNAS[i]]);
		...
		get properties() {
			return {
				...
				onRowUpdate: function (
					...
					// 		misvalores = operar(
					// 			oldValues[0][cambios[0]],
					// 			newValues[0][cambios[0]],
					// 			row[0],
					// 			cambios[0],
					// 			"A"
					// 		);
					// 		confirm(true);
					// 		calculapesos(row[0], "A");
					// 	}
					// 	confirm(true);
				},
