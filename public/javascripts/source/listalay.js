...
$(document).ready(function() {
  ...
  const resumen2 = document.getElementById('resumen2');
    ...
    const Lcabecera = [
      { label:  'Id',  dataField: 'id', align: 'right', cellsAlign: 'center', dataType: 'number', visible: false, cellsFormat: 'n'},
      { label: 'Empresa', dataField: 'empresa', dataType: 'string',allowEdit: false, cellsAlign: 'center'},
      { label: 'Nombre', dataField: 'nombre', dataType: 'string',allowEdit: false},
      { label: 'Ejercicio', dataField: 'ejercicio_descripcion', dataType: 'string',allowEdit: false, cellsAlign: 'center'},
      { label: 'Codigo', dataField: 'codigo', dataType: 'string',allowEdit: false, cellsAlign: 'center'},
      { label: 'Version', dataField: 'version', dataType: 'string',allowEdit: false, cellsAlign: 'center',formatFunction(settings) {
        if(settings.row.data.version == 'VP') {
          settings.template = '<h6><span class="badge rounded-pill bg-primary">'+settings.row.data.version+' - Presupuesto</span></6>';
        }
        if(settings.row.data.version == 'V1') {
          settings.template = '<h6><span class="badge rounded-pill bg-success">'+settings.row.data.version+ ' - Optimista</span></6>';
         
        }
        if(settings.row.data.version == 'V2') {
          settings.template = '<h6><span class="badge rounded-pill bg-danger">'+settings.row.data.version+' - Pesimista</span></6>';
        }
        if(settings.row.data.version == 'V3') {
          settings.template = '<h6><span class="badge rounded-pill bg-warning">'+settings.row.data.version+' - Realista</span></6>';
        }

      }},
      { label: 'Descripcion', dataField: 'descripcion', dataType: 'string',allowEdit: true,width: 300 },
      { label: 'Fecha', dataField: 'fecha', dataType: 'string',cellsAlign: 'center',allowEdit: false},
      { label: 'Fecha Act.', dataField: 'fechaactualizacion', dataType: 'string',cellsAlign: 'center',allowEdit: false},
      { label: 'Usuario', dataField: 'usuario', dataType: 'string',allowEdit: false},
      { label: 'Observaciones',  dataField: 'observaciones', align: 'left', dataType: 'string',width: 300},
      { label:  'Accion',  dataField: 'accion', align: 'center', cellsAlign: 'center',allowEdit: false,
    ...
    template: function (formatObject) {
      if (!formatObject.template) {
          ...
          plus.innerHTML = '<a class="btn btn-default" href="#"><i class="fa fa-folder-open fa_custom" title="Mantener Objetivos"></i></a> <a class="btn btn-default" href="/empresas/empresaslistado"><i class="fa fa-flag fa_custom " title="Align Center"></i> </a>';
          ...
          const buttons = formatObject.template.querySelectorAll('.btn-group');
          buttons[0].row = formatObject.row;
      }
  },
}, 
      { label:  'campo_resumen',  dataField: 'campo_resumen', dataType: 'string', visible: false}
];
grid.messages = {
  'en': {  
  "find": "Buscar ....",
        ...
        onCellUpdate: function(cell, oldValue, newValue, confirm) {
          ...
          var columna =  cell[0].column.dataField;
            ...
            $.ajax({ 
                ...
                dataType: 'json',
                cache: false, 
                data: { empresa_id: fila.data.empresa, layout_id: fila.data.id,col: columna,newvalor: newValue },  
...
grid.addEventListener('rowExpand', function (event) {
  const detail = event.detail,
      ...
      grid.setRowStyle(erow.id,{"background":COLORES[erow.level]});
      if (erow.id > 1) {
...
grid.addEventListener('rowCollapse', function (event) {
  const detail = event.detail,
      ...
      if ( erow.level == 0) {
        grid.setRowStyle(erow.id,{"background":"default"});
      }
}),
        ...
        seriesGroups: [
            {
                ...
                valueAxis: 
                  { unitInterval: 0,
                    ...
                    axisSize: 'auto',
                    formatSettings: {   thousandsSeparator: '.',decimalSeparator: ','},
                },
                
                series: [
                    { dataField: 'Valor', displayText: 'Objetivo',labels: { formatSettings: {   thousandsSeparator: '.',decimalSeparator: ','}
...
var abrirarbol = function (fila,all,color) {
  ...
  grid.setRowStyle(fila.id,{"background":COLORES[fila.level]});
    fila.children.forEach(hijo => {
      if (color) {
        //grid.setRowStyle(hijo.id,{"background":COLORES[hijo.level],"color": "purple"});
        grid.setRowStyle(hijo.id,{"background":COLORES[hijo.level],"color": "default"});
      }
      ...
      $.ajax({ 
        ...
        success: function(datos) {
          ...
          for (var i = 0; i < resumentext.length; i++) {
            campos += '<tr><td>'+resumentext[i].nivel+'</td>'+ '<td>'+resumentext[i].descripcion+'</td>'+'<td  class="text-center">'+resumentext[i].count+'</td></tr>';
          }
          
          resumen.dataSource = {
            linea1: '',
            linea2: '<div><table class="table" ></tr> <th> Nivel </th> <th> Campo </th>  <th class="text-center"> # Reg. x Nivel </th></tr>'+campos+'</table> </div>',
            linea3: ''
          };
          $.ajax({ 
            ...
            success: function(datos) {
              ...
              chart.caption =  'Volúmen Agrupado por '+' X  '+resumentext[1].descripcion;
          ...
          $.ajax({ 
            ...
            dataType: 'json',
            cache: false, 
            data: { empresa_id: filaid.empresa, layout_id: filaid.id,nivel_id: 1,version: filaid.version},  
            success: function(data) {
              
              var nivel = '# '+resumentext[0].descripcion;
              cabecera = [
                { label: columnaResumen, dataField: 'Intervalo', dataType: 'string',with: 60},
